...
$res_sql        = tnsb_get_sql_content_to_display( $db, $userId, $contest );

/* echo $sql_hot_not = "SELECT a.id AS ab, b.id AS cd
                FROM (
                    SELECT a.id
                    FROM content a
                    WHERE a.contest = " . $contest . "
                    ORDER BY RAND()
                    limit 0,1
                ) as a,
                (
...
$res_hot_not_ids = mysqli_query($db, $sql_hot_not);

echo '<pre>';
//var_dump($res_hot_not_ids);
print_r(mysqli_fetch_all( $res_hot_not_ids, MYSQLI_ASSOC ));
exit; */
...
function tnsb_get_sql_content_to_display( $db, $userId, $contest, $parsed_ids=array() ) {
    ...
    if (
        !$res_hot_not_ids ||
        mysqli_num_rows($res_hot_not_ids) == '0' ||
        mysqli_num_rows($res_hot_not_ids) == '1'
    ) {
        return 'aaa:bbb:hhh';
    }

    $res        = mysqli_fetch_all( $res_hot_not_ids, MYSQLI_ASSOC );
    ...
    if ( '' != $userId ) {
			...
			$sql_parsed_ids 		= '';
			if ( !empty( $parsed_ids ) ){
				$sql_parsed_ids 	= "AND (r.photo_id, r.shown_with_photo_id) NOT IN (" . $ids . ")";
			}


			$inner_sub_query    = "SELECT r.photo_id, r.shown_with_photo_id
															FROM ratings r
															...
															WHERE
																	...
																	AND r.photo_id = '" . $id1 . "'
																	AND r.shown_with_photo_id = '" . $id2 . "'
																	. $sql_parsed_ids
														";

		$res_inner_sub_query = mysqli_query($db, $inner_sub_query);

		if ( $res_inner_sub_query ) {
			echo '<hr>In if<hr>';
			return tnsb_get_sql_content_to_display( $db, $userId, $contest, $ids );
		}

		$res        = mysqli_fetch_all( $res_inner_sub_query, MYSQLI_ASSOC );
		...
		echo $sql_hot_not = "SELECT c.photo, c.id, c.elo_score, u.name, u.email, u.user, u.profile_picture
										FROM content c
